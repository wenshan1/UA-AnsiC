...
OpcUa_StatusCode OpcUa_WssListener_ConnectionManager_Create(
    OpcUa_WssListener_ConnectionManager** a_ppConnectionManager)
{
    OpcUa_WssListener_ConnectionManager *pConnMngr  = OpcUa_Null;
    ...
    *a_ppConnectionManager = OpcUa_Null;
    ...
    pConnMngr = (OpcUa_WssListener_ConnectionManager*)OpcUa_Alloc(sizeof(OpcUa_WssListener_ConnectionManager));
    OpcUa_ReturnErrorIfAllocFailed(pConnMngr);

    OpcUa_WssListener_ConnectionManager_Initialize(pConnMngr);

    if(pConnMngr->Connections == OpcUa_Null)
    {
        OpcUa_WssListener_ConnectionManager_Delete(&pConnMngr);
    }
...
OpcUa_StatusCode OpcUa_WssListener_ConnectionManager_Initialize(
    OpcUa_WssListener_ConnectionManager* a_pConnectionManager)
{
    ...
    OpcUa_MemSet(a_pConnectionManager, 0, sizeof(OpcUa_WssListener_ConnectionManager));
    ...
    OpcUa_WssListener_ConnectionManager** a_ppConnectionManager)
{
    ...
    {
        OpcUa_WssListener_ConnectionManager_Clear(*a_ppConnectionManager);
...
OpcUa_StatusCode OpcUa_WssListener_ConnectionManager_GetConnectionBySocket(
    ...
    OpcUa_WssListener_Connection**          a_ppConnection)
{
    OpcUa_StatusCode                uStatus         = OpcUa_Good;
    OpcUa_WssListener_Connection*   tmpConnection   = OpcUa_Null;
    ...
    *a_ppConnection = OpcUa_Null;
    ...
    tmpConnection = (OpcUa_WssListener_Connection*)OpcUa_List_GetCurrentElement(a_pConnectionManager->Connections);

    while(tmpConnection != OpcUa_Null)
    {
        OPCUA_P_MUTEX_LOCK(tmpConnection->Mutex);

        if (a_pSocket == tmpConnection->Socket)
        {
            *a_ppConnection = tmpConnection;
            tmpConnection->iReferenceCount++;
            OpcUa_Trace(OPCUA_TRACE_LEVEL_DEBUG, "OpcUa_WssListener_ConnectionManager_GetConnectionBySocket: 0x%08X increasing RefCount %u\n", tmpConnection, tmpConnection->iReferenceCount);
            uStatus = OpcUa_Good;
            OPCUA_P_MUTEX_UNLOCK(tmpConnection->Mutex);
            break;
        }

        OPCUA_P_MUTEX_UNLOCK(tmpConnection->Mutex);

        tmpConnection = (OpcUa_WssListener_Connection *)OpcUa_List_GetNextElement(a_pConnectionManager->Connections);
    }
    ...
    OpcUa_WssListener_Connection*           a_pConnection)
{
    ...
    a_pConnection->iReferenceCount++;

    OpcUa_List_Enter(a_pConnectionManager->Connections);
    OpcUa_List_AddElement(a_pConnectionManager->Connections, a_pConnection);
    OpcUa_Trace(OPCUA_TRACE_LEVEL_DEBUG, "OpcUa_WssListener_ConnectionManager_AddConnection: Connection added!\n");
    OpcUa_List_Leave(a_pConnectionManager->Connections);
...
OpcUa_StatusCode OpcUa_WssListener_ConnectionManager_RemoveConnections(
    OpcUa_WssListener_ConnectionManager* a_pConnectionManager,
    OpcUa_WssListener_ConnectionDeleteCB a_fConnectionDeleteCB)
{
    OpcUa_StatusCode                uStatus         = OpcUa_Good;
    OpcUa_WssListener_Connection*   tcpConnection   = OpcUa_Null;
    ...
    OpcUa_List_ResetCurrent(a_pConnectionManager->Connections);
    tcpConnection = (OpcUa_WssListener_Connection*)OpcUa_List_GetCurrentElement(a_pConnectionManager->Connections);
    ...
    while(tcpConnection != OpcUa_Null)
    {
        ...
        OpcUa_WssListener_Connection_Delete((&tcpConnection));
...
/*===========================================================================*/
/**
* @brief Allocate and initialize a new OpcUa_WssListener_Connection object.
*
* @return Bad status on fail; Good status on success;
*/
OpcUa_StatusCode OpcUa_WssListener_Connection_Create(OpcUa_WssListener_Connection** a_ppConnection)
{
    OpcUa_WssListener_Connection*   pConnection = OpcUa_Null;

    OpcUa_DeclareErrorTraceModule(OpcUa_Module_TcpListener);

    pConnection = (OpcUa_WssListener_Connection*) OpcUa_Alloc(sizeof(OpcUa_WssListener_Connection));
    OpcUa_ReturnErrorIfAllocFailed(pConnection);
    OpcUa_MemSet(pConnection, 0, sizeof(OpcUa_WssListener_Connection));

    pConnection->uCurrentChunk  = 0;

    OpcUa_WssListener_Connection_Initialize(pConnection);
...
* @return Bad status on fail; Good status on success;
*/
OpcUa_StatusCode OpcUa_WssListener_Connection_Initialize(OpcUa_WssListener_Connection* a_pConnection)
{
    OpcUa_StatusCode uStatus    = OpcUa_Good;
    OpcUa_DeclareErrorTraceModule(OpcUa_Module_TcpListener);
    ...
    a_pConnection->eState                   = OpcUa_WssConnection_StreamState_Created;
...
/*==============================================================================*/
/**
* @brief Clean up the given OpcUa_WssListener_Connection. May be used for static memory.
*/
OpcUa_Void OpcUa_WssListener_Connection_Clear(OpcUa_WssListener_Connection* a_pConnection)
{
    ...
    {
        OpcUa_WssStream_Close((OpcUa_Stream*)a_pConnection->pInputStream);
        OpcUa_WssStream_Delete((OpcUa_Stream**)&a_pConnection->pInputStream);
    }
    ...
    OpcUa_Trace(OPCUA_TRACE_LEVEL_DEBUG, "OpcUa_WssListener_Connection_Clear: Done!\n");
}
...
/*==============================================================================*/
/**
* @brief Clean up and free the given OpcUa_WssListener_Connection.
*/
OpcUa_Void OpcUa_WssListener_Connection_Delete(OpcUa_WssListener_Connection** a_ppConnection)
{
    ...
    /* dont clean up unless all references released */
    if ((*a_ppConnection)->iReferenceCount <= 0)
    {
        /* clean up internal resources */
        OpcUa_WssListener_Connection_Clear(*a_ppConnection);
    ...
    OpcUa_WssListener_Connection**          a_ppConnection)
{
    ...
    OPCUA_P_MUTEX_LOCK((*a_ppConnection)->Mutex);

    (*a_ppConnection)->iReferenceCount--;

    if ((*a_ppConnection)->iReferenceCount <= 0)
    {
        OpcUa_Trace(OPCUA_TRACE_LEVEL_DEBUG, "OpcUa_WssListener_ConnectionManager_ReleaseConnection: Deleting 0x%08X -- RefCount %u\n", (*a_ppConnection), (*a_ppConnection)->iReferenceCount);
        OPCUA_P_MUTEX_UNLOCK((*a_ppConnection)->Mutex);
        OpcUa_WssListener_Connection_Delete(a_ppConnection);
        uStatus = OpcUa_Good;
    }
    else
    {
        OpcUa_Trace(OPCUA_TRACE_LEVEL_DEBUG, "OpcUa_WssListener_ConnectionManager_ReleaseConnection: 0x%08X -- RefCount %u\n", (*a_ppConnection), (*a_ppConnection)->iReferenceCount);
        OPCUA_P_MUTEX_UNLOCK((*a_ppConnection)->Mutex);
    }
